...
#GENERAL PARAMETERS
env_data_folder<-"./Environmental Parameters/Res 01/Historical 2019"
presence_data_folder<-"./Presence and Absence records/Presence Records/Fish"
absence_data_folder<-"./Presence and Absence records/Absence Records/Fish"
presence_data_folder_nf<-"./Presence and Absence records/Presence Records/Non Fish"
absence_data_folder_nf<-"./Presence and Absence records/Absence Records/Non Fish"

output_folder<-"./ENM_res_01_2019_vip"
...
allspp<-vip_spp$species_name
  ...
  kernellist <- c("linear", "polynomial","radial") #sigmoid kernel is removed from options, linear is used as comparison
...
for (species in allspp){
  ...
  for (file in all_asc_files){
    ...
    if (length(grid_of_points)==0){
      ...
      af<-paste0(absence_data_folder,"/",absence_file)
      if (!file.exists(af)){
        af<-paste0(absence_data_folder_nf,"/",absence_file)
      }
      absence<-read.table(af,header = TRUE, sep=",")
