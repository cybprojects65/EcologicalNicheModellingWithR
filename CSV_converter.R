...
cat("\nCSV data format conversion and metadata.txt creation\n")

Workflow_Parameters_File<- paste0("./Workflow_Parameters.txt")
WPFprops <- read.properties(Workflow_Parameters_File)

#input parameters
method <- WPFprops$"method"
resolution <- as.double(WPFprops$"resolution")
set_threshold <- as.double(WPFprops$"set_threshold")
changing_thr <- as.logical(WPFprops$"changing_thr")

#general parameters
main_folder<-"."
CSV_input_folder <- "./CSV"
ENV_parameters_folder <- "./Environmental Parameters/Training"
...
all_IDs<-list.files(CSV_input_folder)
...
for(ID in all_IDs){
  ...
  #changing threshold
  if(changing_thr == T){
     BMPFile<- paste0(CSV_input_folder,"/",ID,".txt")
  ...
  text_lines <- c(paste0("ID name = ",ID),
                  paste0("\nModel = ",method),
  ...
  datasp <- read.csv(paste0(CSV_input_folder,"/",ID,".csv"))
